...
export default function PaymentCallbackPage() {
  ...
  useEffect(() => {
    const verifyPayment = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/paystack/callback?reference=${reference}`, {
          credentials: 'include',
        });
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[var(--light-bg)] px-4 py-12 text-center space-y-6">
      ...
      {status === 'success' && (
        <>
          ...
          <Link href="/dashboard/my-orders" className="px-6 py-3 rounded-lg bg-[var(--acc-clr)] text-white font-medium">
            View Orders
          </Link>
        </>
      )}
      {status === 'failed' && (
        <>
          ...
          <Link href="/explore" className="px-6 py-3 rounded-lg border border-red-500 text-red-500 font-medium hover:bg-red-500 hover:text-white">
            Try Again
          </Link>
        </>
      )}
